...
use ruler::{
    ...
    recipe_utils::{iter_metric, LLMUsage},
};

pub async fn best_enumo_recipe(llm_usage: LLMUsage) -> Ruleset<Math> {
    let mut rules = replicate_ruler1_recipe(llm_usage.clone()).await;
