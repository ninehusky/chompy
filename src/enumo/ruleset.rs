...
impl<L: SynthLanguage> Ruleset<L> {
    ...
    fn get_canonical_conditional_rule(
        ...
        if egraph.find(l_id) == egraph.find(r_id) {
            ...
            println!("l_expr: {l_expr}");
            println!("r_expr: {r_expr}");
            return None;
        }
...
#[cfg(test)]
mod ruleset_tests {
    use crate::enumo::{Metric, Workload};
    use crate::halide::Pred;
    use crate::recipe_utils::{recursive_rules_cond, Lang};
    ...
    #[test]
    fn no_and_1_conditions() {
        let conditions = Workload::new(&["(<= a b)", "(&& 1 (<= a b))"]);

        let mut base_implications: ImplicationSet<Pred> = Default::default();
